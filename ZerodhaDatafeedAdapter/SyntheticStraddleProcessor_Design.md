...
The SSP will operate as an internal service within the ZerodhaDatafeedAdapter process. It will not establish its own external data connections but will leverage the ZerodhaDatafeedAdapter's existing market data reception infrastructure (e.g., Connector.cs).

Code snippet

graph TD
    A[External Data Source/Broker] -- Market Data Stream --> B(Connector.cs / ZerodhaAdapter.cs: Tick Receiver);
    B -- Raw Ticks (All Instruments) --> C{ZerodhaAdapter.cs: Market Data Dispatcher};

    subgraph SSP Subsystem (within ZerodhaAdapter.cs process)
    ...
    G(ZerodhaAdapter.cs: Synthetic Tick Publisher) -- Formatted Synthetic Tick --> D[NinjaTrader Core: Market Data Engine];
    C -- Unrelated/Regular Instrument Ticks --> D;
Detailed Flow:

External Data Source/Broker: Transmits raw market data ticks.
Connector.cs / ZerodhaAdapter.cs (Tick Receiver): The ZerodhaDatafeedAdapter's existing component responsible for receiving all incoming ticks from the broker.
ZerodhaAdapter.cs (Market Data Dispatcher):
...
Receives leg ticks from ZerodhaAdapter.cs.
...
Triggers the ZerodhaAdapter.cs to publish this newly calculated synthetic price as a tick.
ZerodhaAdapter.cs (Synthetic Tick Publisher):
...
namespace ZerodhaDatafeedAdapter.SyntheticInstruments
{
    // Define a basic TickType enum if not already present in ZerodhaAdapter
...
namespace ZerodhaDatafeedAdapter.SyntheticInstruments
{
    ...
    {
        private readonly ZerodhaAdapter _ZerodhaAdapterInstance; // Reference to the ZerodhaAdapter for publishing
        ...
        // Event to notify ZerodhaAdapter about a new synthetic tick
        // public event Action<string, double, DateTime, long, TickType> OnSyntheticTickReady;

        public SyntheticStraddleService(ZerodhaAdapter ZerodhaAdapterInstance)
        {
            _ZerodhaAdapterInstance = ZerodhaAdapterInstance ?? throw new ArgumentNullException(nameof(ZerodhaAdapterInstance));
        ...
        public void LoadStraddleConfigs(string configFilePath)
        {
            ...
            {
                ZerodhaLogger.Log($"Straddle config file not found: {configFilePath}", LogLevel.Error);
                return;
            }
            ...
            try
            {
                ...
                {
                    ZerodhaLogger.Log($"No straddle definitions found in {configFilePath}.", LogLevel.Warning);
                    return;
                }
                ...
                foreach (var def in definitions)
                {
                    ...
                    ZerodhaLogger.Log($"Loaded straddle: {def.SyntheticSymbolNinjaTrader} (CE: {def.CESymbol}, PE: {def.PESymbol})", LogLevel.Info);
                }
            }
            catch (JsonException ex)
            {
                ZerodhaLogger.Log($"Error parsing straddle config file {configFilePath}: {ex.Message}", LogLevel.Error);
            }
            catch (Exception ex)
            {
                ZerodhaLogger.Log($"An unexpected error occurred loading straddle configs: {ex.Message}", LogLevel.Error);
            }
        }
        ...
        public void ProcessLegTick(Tick tick)
        {
            ...
            foreach (var state in affectedStates)
            {
                ...
                if (priceUpdated && state.HasCEData && state.HasPEData)
                {
                    ...
                    _ZerodhaAdapterInstance.PublishSyntheticTickData(
...
5. Integration with ZerodhaDatafeedAdapter (C# Specifics)
This section details how ZerodhaAdapter.cs interacts with the SSP.

5.1. Folder Structure and Class Placement
Maintain the proposed folder structure:

ZerodhaDatafeedAdapter/SyntheticInstruments/StraddleDefinition.cs
ZerodhaDatafeedAdapter/SyntheticInstruments/StraddleState.cs
ZerodhaDatafeedAdapter/SyntheticInstruments/Tick.cs (if not already defined elsewhere)
ZerodhaDatafeedAdapter/SyntheticInstruments/SyntheticStraddleService.cs
5.2. Initialization & Lifetime in ZerodhaAdapter.cs
The SyntheticStraddleService instance should be created and initialized during the ZerodhaAdapter's setup phase.
...
using ZerodhaDatafeedAdapter.SyntheticInstruments; // New using directive

// Assuming ZerodhaLogger and LogLevel exist from your previous design
public static class ZerodhaLogger
{
...
// Assuming ZerodhaAdapter inherits from a suitable NinjaTrader Adapter base class
// The actual base class might be specific (e.g., NinjaTrader.Custom.Adapter.SomeBaseAdapter)
public partial class ZerodhaAdapter // Assuming ZerodhaAdapter is already defined
{
    ...
    // Constructor or suitable initialization method in ZerodhaAdapter
    public ZerodhaAdapter()
    {
        // ... existing ZerodhaAdapter initialization ...
    ...
    public void OnAdapterStarted() // Or equivalent event handler
    {
        ...
        ZerodhaLogger.Log("SyntheticStraddleService initialized and configurations loaded.", ZerodhaLogger.LogLevel.Info);
    }
...
5.3. Tick Forwarding from ZerodhaAdapter.cs
Modify the method in ZerodhaAdapter.cs that receives raw ticks from Connector.cs. This is typically an OnMarketData override or a handler for a MarketData event.

C#

// Inside ZerodhaAdapter.cs

// This method is typically an override from a NinjaTrader base adapter or a handler for your connector's market data event.
// The exact signature depends on your ZerodhaAdapter's base class.
...
protected override void OnMarketData(MarketDataEventArgs marketDataArgs) // Or similar signature
{
    ...
    lock (_marketDataLock)
    {
        ...
        if (_syntheticStraddleService != null && _syntheticStraddleService.IsLegInstrument(marketDataArgs.Instrument.FullName))
        {
            ...
            var tick = new ZerodhaDatafeedAdapter.SyntheticInstruments.Tick
            {
        ...
        else
        {
            ...
            // if you call base.OnMarketData earlier, or if this ZerodhaAdapter directly dispatches.
...
private ZerodhaDatafeedAdapter.SyntheticInstruments.TickType MapNinjaTraderMarketDataType(NinjaTrader.Cbi.MarketDataType ntType)
{
    switch (ntType)
    {
        case NinjaTrader.Cbi.MarketDataType.Last: return ZerodhaDatafeedAdapter.SyntheticInstruments.TickType.Last;
        case NinjaTrader.Cbi.MarketDataType.Bid: return ZerodhaDatafeedAdapter.SyntheticInstruments.TickType.Bid;
        case NinjaTrader.Cbi.MarketDataType.Ask: return ZerodhaDatafeedAdapter.SyntheticInstruments.TickType.Ask;
        case NinjaTrader.Cbi.MarketDataType.Trade: return ZerodhaDatafeedAdapter.SyntheticInstruments.TickType.Trade; // If available
        case NinjaTrader.Cbi.MarketDataType.Quote: return ZerodhaDatafeedAdapter.SyntheticInstruments.TickType.Quote; // If available
        default: return ZerodhaDatafeedAdapter.SyntheticInstruments.TickType.Last; // Default or handle appropriately
    }
}
5.4. Publishing Synthetic Ticks to NinjaTrader (from ZerodhaAdapter.cs)
This is the critical integration point. When SyntheticStraddleService generates a synthetic tick, it calls this method on ZerodhaAdapter.cs to push the data to NinjaTrader.
...
/// </summary>
public void PublishSyntheticTickData(string syntheticSymbol, double price, DateTime timestamp, long volume, ZerodhaDatafeedAdapter.SyntheticInstruments.TickType tickType)
{
    ...
    lock (_marketDataLock)
    {
        ...
        // The 'Instruments' collection is typically managed by your ZerodhaAdapter's base.
        ...
        {
            ZerodhaLogger.Log($"ERROR: Synthetic instrument '{syntheticSymbol}' not found in NinjaTrader's instrument collection. " +
                         "Ensure it's correctly defined in mapped_instruments.json and loaded by NT.", ZerodhaLogger.LogLevel.Error);
            return;
        }
        ...
        if (tickType == ZerodhaDatafeedAdapter.SyntheticInstruments.TickType.Bid) ntMarketDataType = NinjaTrader.Cbi.MarketDataType.Bid;
        else if (tickType == ZerodhaDatafeedAdapter.SyntheticInstruments.TickType.Ask) ntMarketDataType = NinjaTrader.Cbi.MarketDataType.Ask;
        ...
        // The exact method call depends on your ZerodhaAdapter's inheritance and architecture.
        // Common scenarios:
        // a) If ZerodhaAdapter directly inherits from NinjaTrader.Custom.Adapter.Adapter:
        ...
        ZerodhaLogger.Log($"Published synthetic tick for {syntheticSymbol}: P={price:F2}, V={volume}, T={timestamp:HH:mm:ss.fff}", ZerodhaLogger.LogLevel.Debug);
    }
}
5.5. mapped_instruments.json Configuration
The synthetic straddle symbols must be explicitly defined in your mapped_instruments.json (or whatever mechanism ZerodhaDatafeedAdapter uses to inform NinjaTrader about available instruments). This allows NinjaTrader to create the necessary Instrument objects and associate them with incoming data.
...
Concurrency: Using ConcurrentDictionary in SyntheticStraddleService for _straddleStatesBySyntheticSymbol and _legSymbolToAffectedStatesMap is good for thread-safety, as ProcessLegTick might be called concurrently if your ZerodhaAdapter receives ticks on multiple threads.
Locking: The _marketDataLock in ZerodhaAdapter.cs is crucial for thread safety when processing market data and publishing to NinjaTrader. Excessive locking can be a bottleneck, but for critical shared resources like the Instruments collection and the NinjaTrader publishing pipeline, it's necessary.
