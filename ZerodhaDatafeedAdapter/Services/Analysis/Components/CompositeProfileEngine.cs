...
namespace ZerodhaDatafeedAdapter.Services.Analysis.Components
{
    ...
    public class CompositeProfileEngine
    {
        ...
        private void CalculateValueArea(CompositeProfile profile, Dictionary<double, (long BuyVol, long SellVol)> ladder)
        {
            var sortedPrices = ladder.Keys.OrderBy(p => p).ToList();
            double totalVolume = ladder.Values.Sum(v => v.BuyVol + v.SellVol);
            double targetVolume = totalVolume * VALUE_AREA_PERCENT;

            int pocIndex = sortedPrices.IndexOf(profile.POC);
            if (pocIndex < 0) pocIndex = sortedPrices.Count / 2;

            double accumulatedVolume = ladder[sortedPrices[pocIndex]].BuyVol + ladder[sortedPrices[pocIndex]].SellVol;
            int vahIndex = pocIndex;
            int valIndex = pocIndex;
            ...
            {
                if (idx < 0 || idx >= sortedPrices.Count) return 0;
                var entry = ladder[sortedPrices[idx]];
                return entry.BuyVol + entry.SellVol;
            };
        ...
        private void CalculateHVNs(CompositeProfile profile, Dictionary<double, (long BuyVol, long SellVol)> ladder, double currentClose)
        {
            long pocVolume = ladder[profile.POC].BuyVol + ladder[profile.POC].SellVol;
            double hvnThreshold = pocVolume * HVN_RATIO;
            ...
            {
                long vol = kvp.Value.BuyVol + kvp.Value.SellVol;
                if (vol >= hvnThreshold)
                {
                    profile.HVNs.Add(kvp.Key);
                    if (kvp.Key <= currentClose)
