...
QANinjaAdapter serves as a bridge between NinjaTrader 8 and the Zerodha trading platform. It provides access to Indian market data (NSE, BSE) through Zerodha's Kite Connect API, with specialized features for options trading including a Market Analyzer with Option Chain visualization and synthetic straddle instruments.
...
|-----------|-------------|
| **QAAdapter** | NinjaTrader adapter interface - handles Subscribe/Unsubscribe calls |
...
1. Copy `QANinjaAdapter.dll` to `Documents\NinjaTrader 8\bin\Custom\`
2. Copy `QABrokerAPI.dll` and dependencies to the same folder
...
Logs are stored in: `Documents\NinjaTrader 8\QAAdapter\Logs\`

To change log level, create/edit: `Documents\NinjaTrader 8\QAAdapter\log4net.config`
...
Access via: NinjaTrader → New → QA Market Analyzer
...
- Verify subscription with `[QAAdapter] SubscribeMarketData: ADDED callback` logs
