...
namespace TokenGeneratorTest
{
    ...
    public class BreezeApiClient
    {
        ...
        /// <summary>
        /// Make request to V1 API (api.icicidirect.com) - used for legacy endpoints
        /// </summary>
        private async Task<string> MakeRequestAsync(string method, string endpoint, Dictionary<string, object> body)
        {
            var url = BASE_URL_V1 + endpoint;
            var timestamp = DateTime.UtcNow.ToString("yyyy-MM-ddTHH:mm:ss.000Z");
            var bodyJson = JsonConvert.SerializeObject(body);

            // Calculate checksum
            var checksum = CalculateChecksum(timestamp, bodyJson);

            Console.WriteLine($"    [DEBUG] URL: {url}");
            Console.WriteLine($"    [DEBUG] Body: {bodyJson}");
            Console.WriteLine($"    [DEBUG] Timestamp: {timestamp}");

            // Use WebRequest with reflection hack for GET with body (same as session generation)
            var request = WebRequest.Create(new Uri(url));
            request.ContentType = "application/json";
            request.Method = method.ToUpper();

            // Add headers
            ((HttpWebRequest)request).Headers.Add("X-Checksum", $"token {checksum}");
            ((HttpWebRequest)request).Headers.Add("X-Timestamp", timestamp);
            ((HttpWebRequest)request).Headers.Add("X-AppKey", _apiKey);
            ((HttpWebRequest)request).Headers.Add("X-SessionToken", _base64SessionToken);
