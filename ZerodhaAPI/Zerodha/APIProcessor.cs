...
using ZerodhaAPI.Common.Caching.Interfaces;
using ZerodhaAPI.Common.Enums;
using ZerodhaAPI.Common.Models.Response.Error;
using System;
...
namespace ZerodhaAPI.Zerodha
{
    public class APIProcessor : IAPIProcessor
    {
        ...
        public APIProcessor(string apiKey, string secretKey, string accessToken, IAPICacheManager apiCache)
        {
            this._apiKey = apiKey;
            ...
            this._logger.Debug((object)$"Zerodha API Processor set up. Cache Enabled={this._cacheEnabled}");
        }
          ...
          BrokerError errorObject)
        {
            ...
            return num < 500 ? new BrokerException("Zerodha API Error", errorObject) : (BrokerException)new BrokerServerException(errorObject);
        }
        ...
        public async Task<T> ProcessGetRequest<T>(BrokerEndpointData endpoint, int receiveWindow = 5000) where T : class
        {
            ...
            switch (endpoint.SecurityType)
            {
                case EndpointSecurityType.None:
                    request2 = await ZerodhaRequestClient.GetRequest(endpoint.Uri);
                    break;
                case EndpointSecurityType.ApiKey:
                    request2 = await ZerodhaRequestClient.ApiKeyGetRequest(endpoint.Uri, this._apiKey);
                    break;
                case EndpointSecurityType.Signed:
                    request2 = await ZerodhaRequestClient.SignedGetRequest(endpoint.Uri, this._apiKey, this._accessToken, this._secretKey, endpoint.Uri.Query);
                    break;
                default:
        ...
        public async Task<T> ProcessDeleteRequest<T>(BrokerEndpointData endpoint, int receiveWindow = 5000) where T : class
        {
            ...
            {
                case EndpointSecurityType.ApiKey:
                    message = await ZerodhaRequestClient.ApiKeyDeleteRequest(endpoint.Uri, this._apiKey);
                    break;
                case EndpointSecurityType.Signed:
                    message = await ZerodhaRequestClient.SignedDeleteRequest(endpoint.Uri, this._apiKey, this._accessToken, this._secretKey, endpoint.Uri.Query);
                    break;
                default:
                    throw new ArgumentOutOfRangeException();
            }
        ...
        public async Task<T> ProcessPostRequest<T>(BrokerEndpointData endpoint, int receiveWindow = 5000) where T : class
        {
            ...
            {
                case EndpointSecurityType.None:
                    message = await ZerodhaRequestClient.PostRequest(endpoint.Uri);
                    break;
                case EndpointSecurityType.ApiKey:
                    message = await ZerodhaRequestClient.ApiKeyPostRequest(endpoint.Uri, this._apiKey);
                    break;
                case EndpointSecurityType.Signed:
                    message = await ZerodhaRequestClient.SignedPostRequest(endpoint.Uri, this._apiKey, this._accessToken, this._secretKey, endpoint.Uri.Query);
                    break;
                default:
                    throw new ArgumentOutOfRangeException();
            }
        ...
        public async Task<T> ProcessPutRequest<T>(BrokerEndpointData endpoint, int receiveWindow = 5000) where T : class
        {
            ...
            {
                case EndpointSecurityType.None:
                    message = await ZerodhaRequestClient.PutRequest(endpoint.Uri);
                    break;
                case EndpointSecurityType.ApiKey:
                    message = await ZerodhaRequestClient.ApiKeyPutRequest(endpoint.Uri, this._apiKey);
                    break;
                case EndpointSecurityType.Signed:
                    message = await ZerodhaRequestClient.SignedPutRequest(endpoint.Uri, this._apiKey, this._accessToken, this._secretKey, endpoint.Uri.Query);
                    break;
                default:
                    throw new ArgumentOutOfRangeException();
            }
